        ...
        { this.state.message.response && ( this.state.message.response.map((item,key)=>
            <div key={key}>
              <div className="listClass" >
                ...
                <div className="middleDiv" onClick={this.onListClick}>
                    <div className="historyTitle">{item.title}</div>
                    <div className="historyTime">{item.notification_time}</div>
                  </div>
                  <img className="deleteIcon" src={require("../Images/delete_icon.png")} /*onClick={this.onClickDelete.bind(this,item.id,item)}*//>
              </div>
            </div>
          )
        )
